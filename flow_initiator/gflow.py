...
class GFlow:
    ...
    def forced_remap(self, link: Template) -> str:
        ...
        for vertex in [link.From, link.To]:
            ...
            if dummy or not remappable:
                output = (
                    f"{namespace}/{vertex.port_name}" if namespace else vertex.port_name
                )
    ...
    def should_skip(self, link: Template) -> bool:
        ...
        ports = {GFlow.__START__, GFlow.__END__}
        ...
        for value in [link.From, link.To]:
            ...
            try:
                ...
                if (
                    self.get_port_template(value.node_inst, value.port_name)
                    in GFlow.__SKIP__
                ):
                    return True
        ...
        while ports:
            ...
            for link in port["links"]:
                ...
                if self.graph[port_to_remap]["remap"] is not None:
                    ...
                    if (
                        ports_already_remapped > 0
                        and self.graph[port_to_remap]["remap"] != remap_to
                    ):
                        error_msg = (
    ...
    def sort_graph(self) -> list:
        ...
        data = sorted(self.graph, key=lambda key: len(self.graph[key]["links"]))
