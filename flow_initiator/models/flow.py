...
from flow_initiator.gflow import GFlow
from dal.helpers import ParamParser
...
class Flow(Model):
    ...
    def _with_prefix(self, prefix: str, nodes: dict, links: dict) -> dict:
        ...
        for _id, value in links:
            ...
            _value["From"] = (
                f"{prefix}{_from}" if _from.upper() != self.__START__ else _from
            )
    ...
    def get_node_inst(self, name: str) -> dict:
        ...
        try:
            ...
            raise KeyError(
                f"Node instance '{name}' does not exist in '{self.ref}'"
            ) from e
    ...
    def get_node_transitions(self, node_inst: str, port_name: str = None) -> set:
        ...
        for link in links:
            ...
            if port_tpl.is_transition(_port_type, _plink.port_type) and filter_by_port(
                ...
                to_transit = (
                    getattr(link["ref"], "From")
                    if _type == "To"
                    else getattr(link["ref"], "To")
                )
    ...
    def get_node_dependencies(
        ...
        for link in links:
            ...
            if (
                self.__START__ == port_from.str.upper()
                or self.__END__ == port_to.str.upper()
            ):
